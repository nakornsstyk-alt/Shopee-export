...
def _scrape_one_input(page, parsed: dict, log_fn, max_pages: int, promo_phrases_json: str) -> list:
    ...
    for page_num in range(max_pages):
        ...
        js_code = (
            ...
            "  // card = largest ancestor of an anchor that still contains no OTHER item id\n"
            "  function __cardOf(anchor){ let item=anchor, probe=anchor.parentElement; for(let up=0; up<12 && probe; up++){ const ids=new Set(); for(const a2 of probe.querySelectorAll('a[href]')){ const i2=__itemId(a2.getAttribute('href')||''); if(i2) ids.add(i2); } if(ids.size>1) break; item=probe; probe=probe.parentElement; } return item; }\n"
            "  function __perMonth(item){ return /ชิ้น\\s*\\/\\s*เดือน|sold\\s*\\/\\s*(month|mo)/i.test(item.innerText||''); }\n"
            "  // group ALL anchors by item id — the same product can appear in more\n"
            "  // than one section (e.g. a lifetime 'ขายแล้ว' rail above the grid AND\n"
            "  // the real per-month grid). Prefer the per-month card so the product\n"
            "  // is kept with its monthly figure instead of the lifetime duplicate.\n"
            ...
            "    let anchor = anchors[0]; let item = __cardOf(anchor);\n"
            "    for (const a of anchors) { const it = __cardOf(a); if (__perMonth(it)) { anchor = a; item = it; break; } }\n"
